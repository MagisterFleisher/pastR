...
Let's face it.  R looks ugly.  It hurts your eyes.  Some features are verbose or overly confusing.  Official R hasn't really changed in the last 20 years.  It could use a couple tweaks:

1) piping as part of the language. ```|>``` instead of ```%>%```
2) allow users to write ```fn``` for ```function``` 
3) create an ```apply``` with an argument to indicate ```[letter]apply```
4) allow ```v()``` to mean vector along with ```c()```
5) transpile base R statements to data.table syntax

Any existing R code easily converts to pastR.  None of the changes require the user to change how they code.  They don't have to learn a new language.
...
Using data.table drasticly improves speed and reduces memory limitations.   The users don't have to remember to include the magrittr package or data.table package.  Those are built into pastR.


Hypothetical Example
------------

Geocoding 1 GB data from a csv with 4 GB RAM
...
coord <- xyData |> geoCode
    |> transform1
    |> apply(., fn(x) { v(x[,1],x[,2]) }, 'data.frame')
...
base R with data.table package
```
library(data.table)
...
?
```
It doesn't matter.  The user has 4 GB.  Their computer keeps crashing.  They probably 32 GB minimum in base R for this data set, 64 GB in tidy.
...
3) ```[letter]apply``` must change.  Myriad varients of ```apply``` turn one of the best features of R into one of the most arcane.  ```apply``` fits perfectly into a vector language.  People with different programming backrounds and people learning R as a first language struggle with the idea of working with a bunch of things at the same time.  It makes sense if you say, "That is your bunch of values.  That is your function.  All you have to do is ```apply``` the function to all of the values."  R requires you specify the output by choosing from a bunch of awkwardly named ```apply``` functions.  The 's' in ```sapply``` probably makes sense given the development of S, but what does 's' have to do with vectors?  Why not specify the type as arguments in the function.  Input type specification can be optional.  Put in a list and want a vector?  ```apply(fn, [list], 'vector')```.  While we're at it, ```mapply``` or ```Map```: choose one.  Why do ```sapply```, ```lapply```, and ```vapply``` exist as separate functions?  This is why arguments exist.  There's much more nonsense in the species of apply, but one more thing.  Why separate mapply from other apply functions?  Why can't all of them apply to as many arguments as long as the function has many arguments?
...
This needs to transplile to R.  It needs to resemble R in almost all ways.  Users should not need to learn a new language.
...
All the letters.  Numbers require moving ones hands.  No numbers.


Some things to ponder
---

- purrr:flatten should be a base R feature.  Add it to pastR

- ``` %in% ``` also needs an equivolant in the pastR specs.
...
- declare an alias for a package namespace. 

- declare alias for functions. Right now you can do that as a hack by putting a function in another function.  That slows things down.
